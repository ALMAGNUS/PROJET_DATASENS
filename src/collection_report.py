...
class CollectionReport:
    def __init__(
        ...
        console: ConsolePrinter | None = None,
    ):
    ...
    def _is_foundation_source(self, source_name: str) -> bool:
        ...
        return (
            "kaggle" in source_lower
            or (
                source_lower == "gdelt_events"
            )  # Seulement gdelt_events (local), pas GDELT_Last15_English (dynamique)
            or ("zzdb" in source_lower)
        )  # TOUTES les sources ZZDB (synthetic + CSV) sont fondation
    ...
    def collect_session_stats(self):
        ...
        for r in c.fetchall():
            ...
            if self._is_foundation_source(source_name) and self._is_foundation_integrated(
                source_name
            ):
        ...
        r = c.fetchone()
        self.stats["session"] = {"total": r[0] or 0, "tagged": r[1] or 0, "analyzed": r[2] or 0}
