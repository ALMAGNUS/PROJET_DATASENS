...
def _enrich_profile(df: pd.DataFrame, stage: str) -> dict:
...
def _build_enrichment_table(root: Path) -> list[dict]:
    ...
    if df_raw is not None:
        rows.append(_enrich_profile(df_raw, "1. RAW"))

    # SILVER: dernier parquet
    ...
    if df_silver is not None:
        rows.append(_enrich_profile(df_silver, "2. SILVER"))

    # GOLD: dernier parquet
    ...
    if df_gold is not None:
        rows.append(_enrich_profile(df_gold, "3. GOLD"))

    # GoldAI merged
    df_goldai = _try_load([goldai_dir / "merged_all_dates.parquet"])
    if df_goldai is not None:
        rows.append(_enrich_profile(df_goldai, "4. GoldAI"))

    # Copie IA (train)
    df_ia = _try_load([ia_dir / "train.parquet"])
    if df_ia is not None:
        rows.append(_enrich_profile(df_ia, "5. Copie IA (train)"))
...
def _scan_trained_models(root: Path) -> list[dict]:
    ...
    for model_dir in sorted(models_dir.iterdir()):
        ...
        # Cherche trainer_state dans le répertoire ou les checkpoints
        state_file = None
        for candidate in [
        ...
        if state_file:
            try:
                ...
                pass
        # Date de modification du config
        try:
            import datetime
...
def main() -> None:
    ...
    with tab_pilotage:
        ...
        with b3:
            copie_ia_topics = st.checkbox("Copie IA: filtrer finance+politique", value=False, key="copie_ia_topics")
