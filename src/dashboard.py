...
class DataSensDashboard:
    ...
    def collect_stats(self):
        ...
        raw_partitions = [p for p in raw_dir.glob("sources_*") if p.is_dir()] if raw_dir.exists() else []
        silver_partitions = [p for p in silver_dir.iterdir() if p.is_dir()] if silver_dir.exists() else []
        gold_partitions = [p for p in gold_dir.glob("date=*") if p.is_dir()] if gold_dir.exists() else []
    ...
    def print_dashboard(self):
        ...
        if self.stats.get('zzdb'):
            ...
            for item in self.stats['zzdb']:
                ...
                console_write(f"   {item['name']:<30s} {active:>8s} {item['total']:>8d} {item['unique']:>8d}")
        ...
        if pct_enriched < 50:
            ...
            console_write("   [INFO] Solution: Lancer 'python scripts/enrich_all_articles.py' pour enrichir tous les articles")
