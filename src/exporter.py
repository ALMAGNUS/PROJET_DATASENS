...
class GoldExporter:
    ...
    def export_all(self, df: pd.DataFrame, partition_date: date = None) -> dict:
        ...
        if 'source' in df.columns:
            zzdb_df = df[df['source'] == 'zzdb_synthetic'].copy()
            if len(zzdb_df) > 0:
                zzdb_path = p_path / 'source=zzdb_synthetic'
                zzdb_path.mkdir(exist_ok=True)
                zzdb_parquet = zzdb_path / 'zzdb_articles.parquet'
                zzdb_csv = self.export_dir / 'gold_zzdb.csv'
                zzdb_df.to_parquet(zzdb_parquet, index=False, engine='pyarrow')
                zzdb_df.to_csv(zzdb_csv, index=False, encoding='utf-8')
