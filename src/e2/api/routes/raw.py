...
async def get_raw_article(article_id: int, current_user: UserInDB = Depends(require_reader)):
    """
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail=f"Article {article_id} not found in RAW zone"
    )
