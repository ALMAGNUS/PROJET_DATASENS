...
echo [5/6] Fine-tuning local model (camembert)
".venv_benchmark\Scripts\python.exe" scripts\finetune_sentiment.py --model camembert --epochs %FT_EPOCHS% --batch-size %FT_BATCH% --max-length %FT_MAXLEN% --max-train-samples %FT_MAX_TRAIN% --max-val-samples %FT_MAX_VAL%
if errorlevel 1 goto :fail
...
echo - models\camembert-sentiment-finetuned
echo - Mode used: %MODE%
