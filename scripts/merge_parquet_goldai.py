...
try:
    ...
    from pyspark.sql.functions import col, row_number, coalesce, to_timestamp, lit
...
class GoldAIMerger:
    ...
    def run_merge(self, force_full: bool = False) -> dict:
        ...
        all_dates = sorted(set(metadata.get("dates_included", [])) | set([d.isoformat() for d in gold_dates]))
        new_metadata = {
