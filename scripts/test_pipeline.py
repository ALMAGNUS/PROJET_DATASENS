...
def test_sentiment_in_db():
    ...
    for label, count, avg_score in results:
        ...
        print(
            f"   {emoji} {label:10s}: {count:4d} articles ({pct:5.1f}%) - Score moyen: {avg_score:.3f}"
        )
    ...
    if without_sentiment > 0:
        ...
        print(
            "   → Solution: Lancez 'python scripts/enrich_all_articles.py' pour enrichir tous les articles"
        )
...
def test_sentiment_in_csv():
    ...
    for sent, count in sentiment_counts.items():
        ...
        print(
            f"   {emoji} {sent:10s}: {count:4d} articles ({pct:5.1f}%) - Score moyen: {avg_score:.3f}"
        )
