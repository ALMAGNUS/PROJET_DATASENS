...
def visualize_sentiment():
    ...
    sentiment_counts.plot(
        kind="bar", ax=ax1, color=[colors.get(s, "#3498db") for s in sentiment_counts.index]
    )
    ...
    for sentiment in ["positif", "neutre", "négatif"]:
        if sentiment in df["sentiment"].values:
            ...
            ax4.hist(
                scores, bins=20, alpha=0.6, label=sentiment, color=colors.get(sentiment, "#3498db")
            )
    ...
    print("     → 4 graphiques dans un seul fichier PNG (2x2)")
    print("     → Fichier avec timestamp pour conserver l'historique")
    ...
    for source in source_sentiment.index:
        ...
        pos = (
            source_sentiment.loc[source, "positif"] if "positif" in source_sentiment.columns else 0
        )
        neg = (
            source_sentiment.loc[source, "négatif"] if "négatif" in source_sentiment.columns else 0
        )
    ...
    if "topic_1" in df.columns:
        ...
        topic_sentiment = (
            df[df["topic_1"] != ""].groupby(["topic_1", "sentiment"]).size().unstack(fill_value=0)
        )
        ...
        for topic in topic_sentiment.index:
            ...
            pos = (
                topic_sentiment.loc[topic, "positif"] if "positif" in topic_sentiment.columns else 0
            )
            neg = (
                topic_sentiment.loc[topic, "négatif"] if "négatif" in topic_sentiment.columns else 0
            )
