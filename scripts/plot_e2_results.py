...
def _load_training_results() -> dict | None:
    ...
    for model_dir in (
        ROOT / "models" / "sentiment_fr-sentiment-finetuned",
...
def _plot_benchmark_overview(df: pd.DataFrame) -> None:
    ...
    fig.suptitle(
        "E2 — Benchmark comparatif des modèles de sentiment", fontsize=13, fontweight="bold"
    )
    ...
    ax0 = axes[0]
    bars_acc = ax0.bar(
        x - w / 2, df["accuracy"], width=w, label="Accuracy", color=colors, alpha=0.95
    )
    bars_f1 = ax0.bar(
        x + w / 2,
        df["f1_macro"],
    ...
    for bar, val in zip(bars_f1, df["f1_macro"], strict=False):
        ax0.text(
            bar.get_x() + bar.get_width() / 2, val + 0.015, f"{val:.3f}", ha="center", fontsize=7
        )
    ...
    ax1 = axes[1]
    bars_lat = ax1.bar(
        x, df["avg_latency_ms"], color=colors, alpha=0.9, edgecolor="black", linewidth=0.5
    )
    ax1.set_xticks(x)
...
def _plot_benchmark_per_class(bench: dict) -> None:
    ...
    b_neg = ax.bar(
        x - w, f1_neg, width=w, label="F1 négatif", color=CLASS_COLORS["neg"], alpha=0.88
    )
    b_neu = ax.bar(x, f1_neu, width=w, label="F1 neutre", color=CLASS_COLORS["neu"], alpha=0.88)
    b_pos = ax.bar(
        x + w, f1_pos, width=w, label="F1 positif", color=CLASS_COLORS["pos"], alpha=0.88
    )
...
def _plot_benchmark_curves(df: pd.DataFrame) -> None:
    ...
    curve_df["latency_eff"] = 1 - (
        (curve_df["avg_latency_ms"] - lat_min) / max(lat_max - lat_min, 1e-9)
    )
    ...
    fig.suptitle(
        "E2 — Profils multi-critères normalisés (qualité, confiance, efficience)",
        fontsize=12,
        fontweight="bold",
    )
...
def _plot_quality_latency_pareto(df: pd.DataFrame) -> None:
    ...
    fig.suptitle(
        "E2 Innovation — Frontière de Pareto qualité / latence", fontsize=13, fontweight="bold"
    )
    ...
    ax.text(
        data["avg_latency_ms"].min(),
        0.51,
        "Seuil qualité cible F1 ≥ 0.50",
        fontsize=8,
        color="#065f46",
    )
    ...
    patches = [
        mpatches.Patch(
            color=MODEL_COLORS.get(m, "#94a3b8"), label=MODEL_LABELS.get(m, m).replace("\n", " ")
        )
        for m in data["model"]
    ]
    ax.legend(
        handles=[*patches, mpatches.Patch(color="#dc2626", label="Frontière de Pareto")],
        fontsize=8.5,
        loc="upper left",
    )
...
def _plot_class_imbalance(bench: dict) -> None:
    ...
    fig.suptitle(
        "E2 — Déséquilibre des classes & impact sur F1 positif", fontsize=13, fontweight="bold"
    )
...
def _plot_training_quick() -> None:
    ...
    fig.suptitle(
        f"E2 Training {mode_label} — Métriques de validation\n({model_name}, {train_n:,} ex., {epochs} epoch(s))",
        fontsize=12,
        fontweight="bold",
    )
    bars = ax.bar(
    ...
    ax.text(
        0.5,
        0.08,
        "Note : class_weight='balanced' appliqué pour le déséquilibre des classes."
        if mode_label == "full"
        else "Note : run quick — class weights actifs.",
        ha="center",
    ...
    fig2.suptitle(
        f"E2 Training {mode_label} — Performance d'exécution CPU\n(mode {mode_label})",
        fontsize=12,
        fontweight="bold",
    )
    ...
    for bar, val in zip(
        bars2b, [throughput, float(tr.get("train_steps_per_second", 0))], strict=False
    ):
        ax2b.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height() + 0.05,
    ...
    ax2b.text(
        0.5,
        0.08,
        f"Dataset: train={train_n:,} | val={val_n:,} | CPU only",
        ha="center",
...
def _plot_training_quick_curves() -> None:
    ...
    fig.suptitle(
        f"E2 Training {mode_label} — Courbe de perte approximée\n({model_name}, {epochs} epoch(s), mode {mode_label})",
        fontsize=12,
        fontweight="bold",
    )
    ...
    ax.plot(
        ...
        linewidth=2.5,
        color="#2563eb",
        label=f"Train loss (final: {train_loss_end:.4f})",
    )
    ax.plot(
        steps,
        eval_curve,
        ...
        color="#dc2626",
        linestyle="--",
        label=f"Eval loss  (final: {eval_loss_end:.4f})",
    )
